...
from wofs_sounding_variable_extractor_RS_Relative import *
...
# 	print("Maximum height of QCed observation data: " + str(hcutoff) + 'm')
	...
	distlvl = np.ones((len(hMeanWoFS),len(alt)))*np.nan
	for i in range(len(hMeanWoFS)):
		for j in range(len(alt)):
			distlvl[i,j] = abs((hMeanWoFS[i]-alt[j]))

	decihgt = np.ones(len(distlvl[:,1]))*np.nan
	decidex = np.ones(len(distlvl[:,1]))*np.nan
	for i in range(len(distlvl[:,1])):
		decidex[i] = np.argmin(distlvl[i,:], axis = None)
		decihgt[i] = alt[decidex[i]]
		if decihgt[i-1] == decihgt[i]:
			repeathgt = i
			break
# 			
# 	for i in range(len(distlvl[:,1])):
...
# 	print("Decicut: " + str(i))
	...
	#All obs data is decimated onto wofs grid. If WoFS grid extends above observations, decihgt elements are set to nan
	decihgt[repeathgt:] = np.nan

	# Defining height window for decimation
	hgtwindow = 20
	hgtindiciesnan = np.ones((len(decihgt),len(alt)))*np.nan
	for i in range(len(hgtindiciesnan)):
		for j in range(len(alt)):
			if (decihgt[i]-hgtwindow) <= alt[j] <= (decihgt[i] + hgtwindow):
				hgtindiciesnan[i,j] = j

	#Decimating wind onto decihgt (averaging nearby winds)
	
	deciU = np.ones(len(decidex))*np.nan
	deciU[0] = np.average(windU[:int(decidex[0])])                                
	for i in range(len(decidex)-1):                                               
		deciU[i+1] = np.average(windU[hgtindiciesnan[i,~np.isnan(hgtindiciesnan[i,:])]])

	deciV = np.ones(len(decidex))*np.nan
	deciV[0] = np.average(windV[:int(decidex[0])])                                 
	for i in range(len(decidex)-1):                                              
		deciV[i+1] = np.average(windV[hgtindiciesnan[i,~np.isnan(hgtindiciesnan[i,:])]])

	decit = np.ones(len(decidex))*np.nan
	decit[0] = np.average(thetaObs[:int(decidex[0])])                             
	for i in range(len(decidex)-1):                                               
		decit[i+1] = np.average(thetaObs[hgtindiciesnan[i,~np.isnan(hgtindiciesnan[i,:])]])

	deciq = np.ones(len(decidex))*np.nan
	deciq[0] = np.average(q[:int(decidex[0])])                                 
	for i in range(len(decidex)-1):                                                
		deciq[i+1] = np.average(q[hgtindiciesnan[i,~np.isnan(hgtindiciesnan[i,:])]])
	...
	ne = 18
	nz = len(deciU)
	outdir = "/home/jordan.laser/WoFS_stats/0517/Relative/RS/FarthestTime"
	outname = "WoFSMemberStats_" + str(date[0]) + "_" + WoFSrun + '_' + str(time[0]) + ".nc"         #output file
	...
	fout.variables['tWoFS'][:] = thetaWoFS
	...
	plt.savefig('plots/Relative/Wind_Plots/RS_ClosestTime/RS_WoFS_hodo_' + str(date[0]) + '_' + WoFSrun + '_' + str(time[0]) + '.png', dpi = 300)
# 	plt.show()
