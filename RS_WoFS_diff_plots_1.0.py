...
#	fig.suptitle('Radiosonde Observation May 17th ' + time + ' UTC')
	...
	plt.ylabel('Alt (m)', rotation = 'horizontal', fontsize = LS)
...
# 	ax2.axhline(y = hWoFSM[mynnind], color = 'm')
	...
	plt.savefig('plots/Wind_Plots/RS_windPBLdiffplt_' + str(date) + '_' + str(time) + '.png', dpi = 300)
# 	
...
#	fig.suptitle('Observation ' + time + 'UTC')
	ax1 = fig.add_subplot(1, 2, 1)
	ax1.set_title('Potential Temperature' , fontsize = LS)
	plt.xlabel('WoFS theta - Obs theta (K)', fontsize = ls)
	ax1.set_yticks(heightticks)
	plt.ylabel('Alt (m)', rotation = 'horizontal', fontsize = LS)
	ax1.axvline(x = 0, color= 'k')
	# ax1.axhline(y = hWoFSM[PBLind], color = 'k')
...
# 	ax1.axhline(y = hWoFSM[mynnind], color = 'm')
	...
	ax1.set_ylim(0,1800)
	ax2 = fig.add_subplot(1 ,2 ,2)
	ax2.set_title('Water Vapor Mixing Ratio', fontsize = LS)
	ax2.axvline(x = 0, color = 'k')
	ax2.set_yticks(heightticks)
	ax2.set_xlim(-10,10)
	# ax2.axhline(y = hWoFSM[PBLind], color = 'k')
...
# 	ax2.axhline(y = hWoFSM[myjind], color = 'b')
# 	ax2.axhline(y = hWoFSM[mynnind], color = 'm')
	plt.xlabel('WoFS qv - Obs qv (Kg/Kg)', fontsize = ls)
	...
	ax1.set_position([box.x0, box.y0, box.width, box.height])
	legend = ax2.legend(loc='lower left', bbox_to_anchor=(1,0), shadow=True)
	plt.savefig('plots/Thermo_Plots/RS_thermoPBLdiffplt_' + str(date) + '_' + str(time) + '.png', dpi = 300)
