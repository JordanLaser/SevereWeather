...
# 		t = n+8
	...
	#Defining grid point for analysis
	#Storm Relative Option
	LocFile = '/home/jordan.laser/StormLocation/Lidar_FarthestTime/2230/' + time + '.csv'
	StormLoc = pd.read_csv(LocFile, skiprows = 1, sep = ',', header = None)
	Memlist = StormLoc[1]
	...
	#Converting WoFS grid to lat/lon
	...
	for i in range(18):
		WoFSStormLon[i] = Wlon[int(WoFSStormX[i]),int(WoFSStormY[i])]
		WoFSStormLat[i] = Wlat[int(WoFSStormX[i]),int(WoFSStormY[i])]
